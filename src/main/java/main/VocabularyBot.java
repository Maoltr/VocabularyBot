...
public class VocabularyBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        ...
        User user = new User(message.getChatId(), connection);
        ...
        if (message.getText().equals("W3edr509bC")){
            step = 2;
        }
        if (message != null && message.hasText()) {
            switch (step) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    sendMsg(message, "Введите текст который вы хотели бы переслать всем пользователям");
                    step = 3;
                    break;
                case 3:
                    sendMessageForAllUsers(message, connection);
                    end(message);
                    break;
            }
        }
    ...
    private void sendMessageForAllUsers(Message message, Connection connection){
        User user = new User(connection);
        LinkedList<Long> users = user.getAllUsers();
        Runnable r = ()->{
            for (int i = 0; i < users.size(); i++) {
                sendMsg(users.get(i), message.getText());
                try {
                    Thread.sleep(1000   );
